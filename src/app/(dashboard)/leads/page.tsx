...
import { Building2, User, Mail, Phone, Calendar, Clock, Tag, MessageSquare, UploadCloud, Eye, Edit3 } from 'lucide-react';
...
export default function LeadsPage() {
  ...
  const loadAllLeads = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/leads/list?t=${Date.now()}`, { cache: 'no-store' });
      if (res.ok) {
  ...
  const getFollowUpBadgeVariant = (status: FollowUpStatus) => {
    switch (status) {
      ...
      case 'No Answer':
      case 'Needs Reschedule': return 'danger';
      default: return 'default';
    }
  };
  ...
  return (
    <div className="space-y-8">
              ...
              <table className="w-full text-left text-sm whitespace-nowrap min-w-[1300px]">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {paginatedLeads.map((lead: any) => (
                    <tr key={lead._id?.toString() || lead.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="px-5 py-4 font-semibold text-slate-200">
                        <Link href={lead.id ? `/leads/${lead.id}` : '#'} className="hover:text-indigo-400 flex items-center gap-2.5">
                      ...
                      <td className="px-5 py-4">
                        <Badge variant={getStatusBadgeVariant(lead.leadStatus)}>{lead.leadStatus}</Badge>
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {(lead.services || []).map((svc: string, i: number) => (
                      ...
                      <td className="px-5 py-4">
                        <Badge variant={getFollowUpBadgeVariant(lead.followUpStatus)}>{lead.followUpStatus}</Badge>
                      </td>
                      ...
                      <td className="px-5 py-4 text-right">
                        ...
                        >
                          {isSupport ? (
                            ...
                            <><Edit3 className="w-3.5 h-3.5" /><span>Edit Details</span></>
                          )}
                        </Link>
                      </td>
                    </tr>
      ...
      {/* Modal rendered outside Card to avoid layout issues */}
      <ImportLeadModal
        isOpen={isModalOpen}
